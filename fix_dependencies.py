"""
Script para corrigir problemas de dependências do OpenAI
"""

import subprocess
import sys

def fix_openai_dependencies():
    """Corrige as dependências do OpenAI."""
    
    print("🔧 Corrigindo dependências do OpenAI...")
    
    # Lista de comandos para executar
    commands = [
        # Desinstala versões problemáticas
        f"{sys.executable} -m pip uninstall -y openai httpx httpcore",
        
        # Reinstala com versões compatíveis
        f"{sys.executable} -m pip install openai==1.3.0",
        f"{sys.executable} -m pip install httpx==0.25.0",
        
        # Reinstala outras dependências
        f"{sys.executable} -m pip install pinecone-client==2.2.4 python-dotenv==1.0.1 tqdm==4.66.4"
    ]
...
def alternative_fix():
    """Correção alternativa usando requests."""
    print("\n🔄 Aplicando correção alternativa...")
    
    # Cria arquivo ingest_fixed.py com implementação usando requests
...
'''
    ...
    with open('ingest_fixed.py', 'w', encoding='utf-8') as f:
        f.write(fixed_code)
    
    print("✅ Arquivo ingest_fixed.py criado!")
    print("\n🚀 Para usar:")
    print("   python ingest_fixed.py --file data/processed/0a1efcf5_Resultado_Consulta__Comunidade_2024_-_Discentes.txt")

def main():
    print("🔧 CORREÇÃO DE DEPENDÊNCIAS OPENAI")
    print("=" * 50)
    
    # Tenta correção principal
    if fix_openai_dependencies():
        print("\n✅ Dependências corrigidas com sucesso!")
        print("\n🚀 Tente executar novamente:")
        print("   python ingest.py --file data/processed/0a1efcf5_Resultado_Consulta__Comunidade_2024_-_Discentes.txt")
    else:
        print("\n⚠️  A correção principal não funcionou.")
        alternative_fix()
